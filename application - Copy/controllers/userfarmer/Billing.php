...
class Billing extends MY_Controller
{
    ...
    function getPaymentHistory()
    {
        ...
        $query = $this->db->query("SELECT DATE_FORMAT(paid_at,'%d-%m-%Y') date_paid, payment_for,total_payment, status, invoice_no AS ref FROM ($billing) 
                                    WHERE farmer_id=$farmer_id
                                    AND is_paid IS TRUE
                                    AND CONCAT(DATE_FORMAT(paid_at,'%d-%m-%Y'),payment_for, total_payment, status, invoice_no) COLLATE utf8mb4_general_ci LIKE '%$searchValue%'
                                    ORDER BY DATE_FORMAT(paid_at,'%d-%m-%Y') DESC
