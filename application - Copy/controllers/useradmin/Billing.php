...
class Billing extends MY_Controller
{
    ...
    function getBillingHistory()
    {
        ...
        $totalRecords = $thisQuery->row()->total;
        $query = $this->db->query("SELECT id, DATE_FORMAT(paid_at,'%m-%d-%Y %h:%i%p') as paid_at, proof_img_path, DATE_FORMAT(created_at,'%m-%d-%Y %h:%i%p') as billing_date, farmer,DATE_FORMAT(paid_at,'%d-%m-%Y') date_paid, payment_for,total_payment, status, status_remarks, invoice_no AS ref FROM ($billing) 
                                    WHERE is_paid IS TRUE
                                    AND CONCAT(DATE_FORMAT(paid_at,'%d-%m-%Y'),payment_for, total_payment, status, invoice_no) COLLATE utf8mb4_general_ci LIKE '%$searchValue%'
