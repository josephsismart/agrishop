...
class Pending extends MY_Controller
{
    ...
    public function index()
    {
        ...
        // If farmer/supplier — check if already approved; redirect if so
        if ($level == 2) {
            $farmer = $this->db->query("
                SELECT approved_by_person_id, id FROM farmer
                WHERE person_id = ? AND is_active = 1 LIMIT 1
            ", [$person_id])->row();

            if ($farmer && $farmer->approved_by_person_id) {
                $this->session->set_userdata(['agrishop_login_farmer_id' => $farmer->id]);
            ...
            if ($supplier && $supplier->approved_by_person_id) {
                $this->session->set_userdata(['agrishop_login_supplier_id' => $supplier->id]);
                redirect(base_url('usersupplier/dashboard'));
                return;
            }
        } elseif ($level == 1) {
            // Customer - no pending needed
    ...
    // ── Called via AJAX from Pending page every 30s ───────────
    public function checkApprovalStatus()
    {
        ...
        if ($level == 2) {
            ...
            $row = $this->db->query("
                ...
                FROM farmer f WHERE f.person_id = ? AND f.is_active = 1 LIMIT 1
            ", [$person_id])->row();

            if ($row && $row->approved_by_person_id) {
                // Update session so they land correctly after redirect
            ...
            $row = $this->db->query("
                ...
                FROM supplier s WHERE s.person_id = ? AND s.is_active = 1 LIMIT 1
            ", [$person_id])->row();
...
/* Location: ./application/controllers/system/Pending.php */
